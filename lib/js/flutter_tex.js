...
function createTeXViewGroup(element, rootData) {
    ...
    rootData['data'].forEach(function (data) {
        ...
        if (single) {
            // Single selection
            if (id === programmaticallySelectedId) {
                item.setAttribute("style", selectedStyle);
                lastSelected = item;
            }
        }
        else {
            if (arrayContains(programmaticallySelectedIds, id)) {
                item.setAttribute("style", selectedStyle);
            }
        }
        ...
        clickManager(item, id, rootData['rippleEffect'], function (clickedId) {
            if (clickedId === id) {
                if (single) {
                    ...
                    if (programmaticallySelectedIds.includes(clickedId)) {
                        // Deselect
                        clickedElement.setAttribute('style', normalStyle);
                        programmaticallySelectedIds.splice(programmaticallySelectedIds.indexOf(clickedId), 1);
                    } else {
                        // Select
                        clickedElement.setAttribute('style', selectedStyle);
                        programmaticallySelectedIds.push(clickedId);
                    }
                    onTapCallback(JSON.stringify(programmaticallySelectedIds));
                }
            }
            renderCompleted();
        });
